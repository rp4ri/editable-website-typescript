...
import { eq, lt, isNotNull, asc, desc, sql, gt, ne, and } from 'drizzle-orm';
 ...
 */
export async function createArticle(title: string, content: string, teaser: string, currentUser: CurrentUser) {
 ...
 * Updates an article
 */
export async function updateArticle(slug: string, title: string, content: string, teaser: string, currentUser: CurrentUser) {
  if (!currentUser) throw new Error('Not authorized');
  ...
  let nextArticle = await db
      ...
      .where(
          and(
              ...
              lt(articles.published_at, publishedAt)
          )
      )
      .orderBy(desc(articles.published_at))
      .limit(1);
...
export async function search(q: string, currentUser: CurrentUser): Promise<SearchResult[]> {
  ...
  const results = await (currentUser
      ? baseQuery
          .where(sql`${articles.title} LIKE ${searchTerm} COLLATE NOCASE`)
      : baseQuery
          .where(
              and(
 ...
 */
export async function createOrUpdatePage(page_id: string, page: any, currentUser: CurrentUser) {
  if (!currentUser) throw new Error('Not authorized');
