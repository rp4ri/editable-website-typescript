<script lang="ts">
	...
	const onDrag = ({ x, y }: { x: number; y: number }) => {
	...
	const getPointOnContainer = ({ x, y }: { x: number; y: number }) => {
		if (!containerRect) {
	...
	const getPointOnImage = ({ x, y }: { x: number; y: number }) => ({
		x: (x + crop.x) / zoom,
		y: (y + crop.y) / zoom
	});

	const setNewZoom = (newZoom: number, point: { x: number; y: number }) => {
